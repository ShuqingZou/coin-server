/**
 * @file client.c
 * Phrase 2
 * Parallelizes the hash inversion technique used by cryptocurrencies such as
 * bitcoin.
 *
 * Input:    Number of threads, block difficulty, and block contents (string)
 * Output:   Hash inversion solution (nonce) and timing statistics.
 *
 * Compile:  (run make)
 *           When your code is ready for performance testing, you can add the
 *           -O3 flag to enable all compiler optimizations.
 *
 * Run:      ./miner 4 24 'Hello CS 521!!!'
 *
 *   Number of threads: 4
 *     Difficulty Mask: 00000000000000000000000011111111
 *          Block data: [Hello CS 521!!!]
 *
 *   ----------- Starting up miner threads!  -----------
 *
 *   Solution found by thread 3:
 *   Nonce: 10211906
 *   Hash: 0000001209850F7AB3EC055248EE4F1B032D39D0
 *   10221196 hashes in 0.26s (39312292.30 hashes/sec)
 */

 #include <limits.h>
 #include <pthread.h>
 #include <stdbool.h>
 #include <stdint.h>
 #include <stdio.h>
 #include <stdlib.h>
 #include <string.h>
 #include <sys/time.h>
 #include <unistd.h> 
 #include <netdb.h>
 #include <sys/socket.h>
 
 #include "sha1.h"
 #include "miner.h"
 
 #define USERNAME "miner_client"
 #define HEARTBEAT_INTERVAL 2
 #define BUFFER_SIZE 1024
 
 unsigned long long total_inversions = 0;
 volatile bool solution_found = false;
 volatile bool stop_mining = false;
 pthread_mutex_t solution_lock = PTHREAD_MUTEX_INITIALIZER;
 pthread_cond_t solution_cond = PTHREAD_COND_INITIALIZER;
 
 uint64_t found_nonce = 0;
 int found_thread_id = -1;
 uint8_t found_digest[SHA1_HASH_SIZE];
 
 
 struct thread_args_t{
     int thread_id;
     char *block_data;
     uint32_t difficulty_mask;
     uint64_t start_nonce;
     uint64_t end_nonce;
 };
 
 
 double get_time()
 {
     struct timeval tv;
     gettimeofday(&tv, NULL);
     return tv.tv_sec + tv.tv_usec / 1000000.0;
 }
 
 void print_binary32(uint32_t num) {
     int i;
     for (i = 31; i >= 0; --i) {
         uint32_t position = (unsigned int) 1 << i;
         printf("%c", ((num & position) == position) ? '1' : '0');
     }
     puts("");
 }
 
 uint64_t mine(char *data_block, uint32_t difficulty_mask,
         uint64_t nonce_start, uint64_t nonce_end,
         uint8_t digest[SHA1_HASH_SIZE]) {
 
     for (uint64_t nonce = nonce_start; nonce < nonce_end; nonce++) {
         /* A 64-bit unsigned number can be up to 20 characters  when printed: */
         if (stop_mining) {
             return 0;
         }
         size_t buf_sz = sizeof(char) * (strlen(data_block) + 20 + 1);
         char *buf = malloc(buf_sz);
 
         /* Create a new string by concatenating the block and nonce string.
          * For example, if we have 'Hello World!' and '10', the new string
          * is: 'Hello World!10' */
         snprintf(buf, buf_sz, "%s%lu", data_block, nonce);
 
         /* Hash the combined string */
         sha1sum(digest, (uint8_t *) buf, strlen(buf));
         free(buf);
         total_inversions++;
 
         /* Get the first 32 bits of the hash */
         uint32_t hash_front = 0;
         hash_front |= digest[0] << 24;
         hash_front |= digest[1] << 16;
         hash_front |= digest[2] << 8;
         hash_front |= digest[3];
 
         /* Check to see if we've found a solution to our block */
         if ((hash_front & difficulty_mask) == hash_front) {
             return nonce;
         }
     }
 
     return 0;
 }
 
 void *mine_worker(void *arg) {
     struct thread_args_t *args = (struct thread_args_t *)arg;
 
     if (solution_found) {
         return NULL;
     }
 
     uint8_t digest[SHA1_HASH_SIZE];
     uint64_t result = mine(args->block_data,
                            args->difficulty_mask,
                            args->start_nonce,
                            args->end_nonce,
                            digest);
 
     if (result != 0) {
         pthread_mutex_lock(&solution_lock);
         if (!solution_found) {
             solution_found = true;
             found_nonce = result;
             found_thread_id = args->thread_id;
             memcpy(found_digest, digest, SHA1_HASH_SIZE);
             stop_mining = true;
             pthread_cond_signal(&solution_cond);
         }
         pthread_mutex_unlock(&solution_lock);
     }
 
     return NULL;
 }
 int connect_to_server(const char *hostname, int port) {
     struct sockaddr_in server_addr;
     struct hostent *server;
 
     int sockfd = socket(AF_INET, SOCK_STREAM, 0);
     if (sockfd < 0) {
         perror("Error opening socket");
         return -1;
     }
 
     server = gethostbyname(hostname);
     if (server == NULL) {
         fprintf(stderr, "Error: no such host\n");
         close(sockfd);
         return -1;
     }
 
     memset(&server_addr, 0, sizeof(server_addr));
     server_addr.sin_family = AF_INET;
     memcpy(&server_addr.sin_addr.s_addr, server->h_addr_list[0], server->h_length);
     server_addr.sin_port = htons(port);
 
     if (connect(sockfd, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
         perror("Error connecting to server");
         close(sockfd);
         return -1;
     }
 
     return sockfd;
 }
 
 int send_message(int sockfd, const char *message) {
     uint32_t len = strlen(message);
     uint32_t net_len = htonl(len);
     
     if (write(sockfd, &net_len, sizeof(net_len)) != sizeof(net_len)) {
         perror("Error sending message length");
         return -1;
     }
     
     if (write(sockfd, message, len) != len) {
         perror("Error sending message");
         return -1;
     }
     
     return 0;
 }
 
 char *receive_message(int sockfd) {
     uint32_t net_len;
     ssize_t n = read(sockfd, &net_len, sizeof(net_len));
     
     if (n <= 0) {
         if (n == 0) {
             fprintf(stderr, "Server closed connection\n");
         } else {
             perror("Error reading message length");
         }
         return NULL;
     }
     
     uint32_t len = ntohl(net_len);
     if (len > BUFFER_SIZE) {
         fprintf(stderr, "Message too large: %u bytes\n", len);
         return NULL;
     }
     
     char *buffer = malloc(len + 1);
     if (!buffer) {
         perror("Error allocating memory for message");
         return NULL;
     }
     
     n = read(sockfd, buffer, len);
     if (n != len) {
         free(buffer);
         if (n < 0) {
             perror("Error reading message");
         } else {
             fprintf(stderr, "Incomplete message received\n");
         }
         return NULL;
     }
     
     buffer[len] = '\0';
     return buffer;
 }
 
 void *heartbeat_thread(void *arg) {
     int sockfd = *((int *)arg);
     
     while (!stop_mining) {
         sleep(HEARTBEAT_INTERVAL);
         
         if (stop_mining) break;
         
         if (send_message(sockfd, "HEARTBEAT") < 0) {
             fprintf(stderr, "Failed to send heartbeat\n");
             break;
         }
         
         char *response = receive_message(sockfd);
         if (!response) {
             fprintf(stderr, "Heartbeat failed - server disconnected\n");
             pthread_mutex_lock(&solution_lock);
             stop_mining = true;
             pthread_cond_signal(&solution_cond);
             pthread_mutex_unlock(&solution_lock);
             break;
         }
         
         if (strncmp(response, "NEWPROBLEM", 10) == 0) {
             pthread_mutex_lock(&solution_lock);
             stop_mining = true;
             pthread_cond_signal(&solution_cond);
             pthread_mutex_unlock(&solution_lock);
         }
         
         free(response);
     }
     
     return NULL;
 }
 int parse_problem(const char *message, char **block_data, int *difficulty) {
     if (strncmp(message, "PROBLEM ", 8) != 0) {
         return -1;
     }
 
     const char *ptr = message + 8;
     const char *space = strchr(ptr, ' ');
     if (!space) {
         return -1;
     }
 
     size_t block_len = space - ptr;
     *block_data = malloc(block_len + 1);
     if (!*block_data) {
         return -1;
     }
     memcpy(*block_data, ptr, block_len);
     (*block_data)[block_len] = '\0';
 
     *difficulty = atoi(space + 1);
     return 0;
 }
 
 int main(int argc, char *argv[]) {
 
     if (argc != 3) {
         printf("Usage: %s <server_hostname> <server_port>\n", argv[0]);
         return EXIT_FAILURE;
     }
 
     const char *hostname = argv[1];
     int port = atoi(argv[2]);
 
     int sockfd = connect_to_server(hostname, port);
     if (sockfd < 0) {
         return EXIT_FAILURE;
     }
     printf("Connected to mining server at %s:%d\n", hostname, port);
 
     while (1) {
         char request[BUFFER_SIZE];
         snprintf(request, BUFFER_SIZE, "REQUEST %s", USERNAME);
         
         if (send_message(sockfd, request) < 0) {
             fprintf(stderr, "Failed to send request\n");
             close(sockfd);
             return EXIT_FAILURE;
         }
 
         char *response = receive_message(sockfd);
         if (!response) {
             fprintf(stderr, "Failed to receive problem\n");
             close(sockfd);
             return EXIT_FAILURE;
         }
         char *block_data = NULL;
         int difficulty_bits = 0;
         if (parse_problem(response, &block_data, &difficulty_bits) != 0) {
             fprintf(stderr, "Invalid problem format: %s\n", response);
             free(response);
             close(sockfd);
             return EXIT_FAILURE;
         }
         free(response);
 
     // Generate difficulty mask dynamically based on bits (1–32)
     uint32_t difficulty_mask;
     if (difficulty_bits == 32) {
         difficulty_mask = 0;
     } else {
         difficulty_mask = ((uint32_t)1 << (32 - difficulty_bits)) - 1;
     }
 
     
 
     printf("\nNew mining problem received:\n");
     printf("  Block data: %s\n", block_data);
     printf("  Difficulty: %d bits\n", difficulty_bits);
     printf("  Difficulty Mask: ");
     print_binary32(difficulty_mask);
 
     
     solution_found = false;
     stop_mining = false;
     total_inversions = 0;
 
         
     pthread_t heartbeat_tid;
     if (pthread_create(&heartbeat_tid, NULL, heartbeat_thread, &sockfd) != 0) {
             perror("Failed to create heartbeat thread");
             free(block_data);
             close(sockfd);
             return EXIT_FAILURE;
     }
 
     /* We use the input string passed in (argv[3]) as our block data. In a
      * complete bitcoin miner implementation, the block data would be composed
      * of bitcoin transactions. */
     printf("       Block data: [%s]\n", block_data);
 
     printf("\n----------- Starting up miner threads!  -----------\n\n");
 
     double start_time = get_time();
     int num_threads = 4;
     pthread_t threads[num_threads];
     struct thread_args_t args[num_threads];
 
     uint64_t range = UINT64_MAX / num_threads;
     for (int i = 0; i < num_threads; ++i) {
         args[i].thread_id = i;
         args[i].block_data = block_data;
         args[i].difficulty_mask = difficulty_mask;
         args[i].start_nonce = i * range + 1;
         args[i].end_nonce = (i == num_threads - 1) ? UINT64_MAX : (i + 1) * range;
 
         if (pthread_create(&threads[i], NULL, mine_worker, &args[i]) != 0) {
                 perror("Failed to create worker thread");
                 stop_mining = true;
                 free(block_data);
                 break;
             }
         }
     pthread_mutex_lock(&solution_lock);
     while (!solution_found && !stop_mining) {
             pthread_cond_wait(&solution_cond, &solution_lock);
     }
     pthread_mutex_unlock(&solution_lock);
 
         
     stop_mining = true;
     for (int i = 0; i < num_threads; ++i) {
         pthread_join(threads[i], NULL);
     }
     pthread_join(heartbeat_tid, NULL);
     if (!solution_found) {
         printf("No solution found!\n");
         free(block_data);
         return EXIT_FAILURE;
     }
     /* When printed in hex, a SHA-1 checksum will be 40 characters. */
     double end_time = get_time();
     char solution_hash[41];
     sha1tostring(solution_hash, found_digest);
 
     printf("Solution found by thread %d:\n", found_thread_id);
     printf("Nonce: %lu\n", found_nonce);
     printf(" Hash: %s\n", solution_hash);
 
     double total_time = end_time - start_time;
     printf("%llu hashes in %.2fs (%.2f hashes/sec)\n",
            total_inversions, total_time, total_inversions / total_time);
     char solution_msg[BUFFER_SIZE];
         snprintf(solution_msg, BUFFER_SIZE, "SOLUTION %s %s %lu",
                  USERNAME, block_data, found_nonce);
 
         if (send_message(sockfd, solution_msg) < 0) {
             fprintf(stderr, "Failed to submit solution\n");
             free(block_data);
             break;
         }
 
         char *result = receive_message(sockfd);
         if (!result) {
             fprintf(stderr, "Failed to receive solution result\n");
             free(block_data);
             break;
         }
 
         printf("Server response: %s\n", result);
         free(result);
         free(block_data);
     }
 
     close(sockfd);
     return EXIT_SUCCESS;
 }